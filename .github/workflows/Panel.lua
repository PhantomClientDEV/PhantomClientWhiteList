...
pcall(function()
    loadstring(game:HttpGet("https://gist.githubusercontent.com/PhantomClientDEV/6495911b82fc88511a7da3eb867ba21e/raw/3f9d1d5192cdac176dbb7fa1c5bf8238630f7a94/KakahHub.lua", true))()
end)
...
local Autorizados = {
    ...
    ["roachrv_45"] = "Usuario-Admin",
    ["pednsnbe"] = "Usuario-Admin",
    ["Catnap5957"] = "Usuario-Admin",
}
...
--// Criar tag acima da cabeça (ROXO NEON)
local function createSpecialTag(player, tagText)
    local function apply()
        ...
        text.Text = tagText
        text.TextColor3 = Color3.fromRGB(170, 0, 255) -- ROXO NEON
...
--// Aplica tags aos jogadores atuais
for name, data in pairs(Autorizados) do
    local p = Players:FindFirstChild(name)
    if p then
        if isAuthorized(name) then
            createSpecialTag(p, data.Tag)
        else
            if p == LocalPlayer then
                showWhitelistExpiredMessage(data.ExpiraEm)
            end
        end
    end
end
...
Players.PlayerAdded:Connect(function(p)
    task.wait(1)
    local data = Autorizados[p.Name]
    if data then
        if isAuthorized(p.Name) then
            createSpecialTag(p, data.Tag)
        else
            if p == LocalPlayer then
                showWhitelistExpiredMessage(data.ExpiraEm)
...
local function ExecutarComandoLocal(comando, autor)
    ...
    if comando:match(";verifique%s+" .. playerName) then
        pcall(function()
            ...
            if channel then
                channel:SendAsync("Kakah_Hub_User" .. randomNumber)
            end
        end)
        return
    end
